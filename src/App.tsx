...
export default function App() {
  ...
  useEffect(() => {
    ...
    const tournamentDoc = subscribeTournamentDoc(selectedTournamentId, (data) => {
      setState(prev => ({ 
        ...prev, 
    ...
    const teamsSub = subscribeTeams(selectedTournamentId, (teams) => {
      setState(prev => {
    ...
    const groupsSub = subscribeGroups(selectedTournamentId, (groups) => {
      setState(prev => {
    ...
    const matchesSub = subscribeMatches(selectedTournamentId, (matches) => {
      setState(prev => {
    ...
    try {
      ...
      if (config.knockoutPairing === 'manual' && config.manualSlots) {
        ...
        rank1s.forEach((r1, idx) => {
          ...
          if (r1 && r2 && dbMatches.length < config.knockoutSize / 2) {
      ...
      if (dbMatches.length > 0) {
        const { error: matchesErr } = await supabase.from('matches').upsert(dbMatches);
  ...
  const advanceToNextRound = async (currentMatchIdx: number, s1: number, s2: number) => {
    ...
    try {
      ...
      if (nextMatchIdx !== -1) {
        ...
        const teamKey = slotInNextMatch === 1 ? 'team1_id' : 'team2_id';
        ...
        if (winnerId !== oldWinnerId) {
          ...
          if (nextMatch && nextMatch.isCompleted) {
            nextMatchUpdate.is_completed = false;
  ...
  const resetTournamentScores = async () => {
    ...
    try {
      ...
      const { error: tourneyErr } = await supabase
        .from('tournaments')
        .update({
          stage: hasGroupMatches ? 'group' : 'setup'
        })
        .eq('id', selectedTournamentId);
